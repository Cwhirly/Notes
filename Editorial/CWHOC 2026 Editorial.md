...
| 英文名      | april          | road              | wing               |
| Luogu 评级 | ~={yellow}普及=~ | ~={cyan}提高=~      | ~={black}省选/NOI-=~ |
| CF 评级    | ~={cyan}1500=~ | ~={magenta}2000=~ | ~={red}2900=~      |
| 分类       | 数学             | 图论，数学             | 树论，数学              |
| 算法标签     | 数学证明           | Tarjan, Ad-hoc    | Kruskal 重构树，FWT    |
| 侧重点      | 思维             | 算法+思维             | 算法                 |
| 题目来源     | EOlymp         | Codeforces        | Revitalize+Pulsar  |
| std      | Revitalize     | Revitalize        | Revitalize         |
...
如果你不会长链剖分，也容易得到一个简单的办法，考虑一个“孤链压缩”的优化方法，如果一个点只有一个儿子，那么此时显然可以直接用儿子的信息转移，不需要重新计算，发现这种优化方法可以避免在极其长的一条单链上做无效的计算。  
这可以帮助我们多通过几个点，然而卡这种优化的方法很简单，只需要一条链，链上每个点再多挂一个儿子就可以避免被优化。  
你可以发现此时其实已经几乎是长链剖分优化 DP 的模板了。  
我们回到 $dp_{u,c}=\displaystyle\bigoplus_{v\in\mathrm{son}_u}dp_{v,c-1}$ 这一步。  
先对树做长链剖分，要求同一条长链上的节点的 DFN 序是连续的。
此时用到一个技巧，我们不把 $dp$ 开成一个二维数组，而是考虑建立一个类似内存池的大数组 $P$，这个数组的长度为 $n$ 即可，因为每个点都至多属于一条长链，故全体长链的大小之和为 $n$。对于一个一维数组 $dp_u$, 我们不开始就直接枚举 $c$，再枚举 $v$，而是直接把 $dp_{son}$ 数组的值直接赋给 $dp_u$，$son$ 是 $u$ 的长儿子。  
显然如果依旧是枚举 $c$ 然后 $dp_{u,c}\gets dp_{son,c}$，时间复杂度还是错误的，因此我们使用内存池进行优化。  
对于每一个 $u$，我们维护一个 $pos_u$，这代表着：$dp_u$ 等价于 $P_{[pos_u,pos_u+|dp_u|-1]}$。那么此时直接继承长儿子信息时，就可以直接令 $pos_u\gets pos_{son}$，这样就可以 $O(1)$ 做了。  
可以证明，此时的总复杂度是 $O(n)$。  
